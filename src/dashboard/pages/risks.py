...
with search_col:
    search_query = st.text_input(
        "Поиск по названию цели", placeholder="Введите часть названия..."
    )
with sort_col:
...
if search_query:
    ...
    filtered_goals = [
        g for g in filtered_goals if query in (g.get("title") or "").lower()
    ]
...
for goal in filtered_goals:
    ...
    with st.expander(
        f"{emoji} {goal['title']} — Риск: {goal['risk_score']:.0%}", expanded=True
    ):
        ...
        if detail and detail.get("projects"):
            ...
            for p in detail["projects"]:
                st.write(
                    f"  - {p['title']} ({status_labels.get(p['status'], p['status'])})"
                )
