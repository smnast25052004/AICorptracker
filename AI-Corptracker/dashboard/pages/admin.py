...
GOAL_STATUS_LABELS = {
    "on_track": "В норме", "at_risk": "Под угрозой", "critical": "Критический",
...
with tab_goals:
    ...
    with st.expander("➕ Создать новую цель", expanded=False):
        with st.form("create_goal", clear_on_submit=True):
            ...
            with g_col2:
                g_status = st.selectbox("Статус", GOAL_STATUSES, format_func=lambda x: GOAL_STATUS_LABELS.get(x, x), key="g_status")
            with g_col3:
                g_priority = st.selectbox("Приоритет", GOAL_PRIORITIES, index=1, key="g_priority")
            ...
            if st.form_submit_button("Создать цель", type="primary"):
                if not g_title:
                    ...
                    result = api_client.create_goal({
                        "title": g_title, "description": g_desc or None,
                        "owner": g_owner or None, "status": g_status,
                        "priority": g_priority, "progress": g_progress,
    ...
    if goals:
        for goal in goals:
            emoji = {"on_track": "🟢", "at_risk": "🟡", "critical": "🔴", "completed": "🔵"}.get(goal["status"], "⚪")
            with st.expander(f"{emoji} {goal['title']}"):
                with st.form(f"edit_goal_{goal['id']}"):
                    eg_title = st.text_input("Название", value=goal["title"])
                    eg_desc = st.text_area("Описание", value=goal.get("description") or "")
                    eg_c1, eg_c2, eg_c3 = st.columns(3)
                    with eg_c1:
                        eg_owner = st.text_input("Ответственный", value=goal.get("owner") or "")
                    with eg_c2:
                        eg_status = st.selectbox(
                            "Статус", GOAL_STATUSES,
                            index=GOAL_STATUSES.index(goal["status"]) if goal["status"] in GOAL_STATUSES else 0,
                            format_func=lambda x: GOAL_STATUS_LABELS.get(x, x),
                        )
                    with eg_c3:
                        eg_priority = st.selectbox(
                            "Приоритет", GOAL_PRIORITIES,
                            index=GOAL_PRIORITIES.index(goal["priority"]) if goal["priority"] in GOAL_PRIORITIES else 1,
                        )
                    eg_c4, eg_c5 = st.columns(2)
                    with eg_c4:
                        eg_progress = st.slider("Прогресс %", 0.0, 100.0, float(goal.get("progress", 0)))
                    with eg_c5:
                        eg_target = st.text_input("Целевая дата", value=goal.get("target_date") or "")
                    ...
                    if save:
                        result = api_client.update_goal(str(goal["id"]), {
                            "title": eg_title, "description": eg_desc or None,
                            "owner": eg_owner or None, "status": eg_status,
                            "priority": eg_priority, "progress": eg_progress,
...
PROJECT_STATUS_LABELS = {
    "active": "Активный", "on_hold": "Приостановлен",
    "completed": "Завершён", "cancelled": "Отменён",
}
...
with tab_projects:
    ...
    with st.expander("➕ Создать проект", expanded=False):
        with st.form("create_project", clear_on_submit=True):
            ...
            with p_c2:
                p_status = st.selectbox("Статус", PROJECT_STATUSES, format_func=lambda x: PROJECT_STATUS_LABELS.get(x, x), key="p_status")
            with p_c3:
                p_goal = st.selectbox("Цель", [""] + goal_ids, format_func=lambda x: goal_options.get(x, "— Не выбрано —"), key="p_goal")
            ...
            if st.form_submit_button("Создать проект", type="primary"):
                if not p_title:
                    ...
                    result = api_client.create_project({
                        "title": p_title, "description": p_desc or None,
                        "lead": p_lead or None, "status": p_status,
                        "progress": p_progress, "goal_id": p_goal or None,
                    })
                    if "error" in result:
                        st.error(f"Ошибка: {result['error']}")
                    else:
        ...
        for proj in projects:
            ...
            with st.expander(f"📁 {proj['title']} — {status_label}"):
                with st.form(f"edit_project_{proj['id']}"):
                    ...
                    ep_desc = st.text_area("Описание", value=proj.get("description") or "")
                    ep_c1, ep_c2, ep_c3 = st.columns(3)
                    with ep_c1:
                        ep_lead = st.text_input("Руководитель", value=proj.get("lead") or "")
                    with ep_c2:
                        ep_status = st.selectbox(
                            "Статус", PROJECT_STATUSES,
                    ...
                    with ep_c3:
                        ...
                        ep_goal = st.selectbox(
                            "Цель", [""] + goal_ids,
                            index=(goal_ids.index(current_goal) + 1) if current_goal in goal_ids else 0,
                            format_func=lambda x: goal_options.get(x, "— Не выбрано —"),
                        )
                    ep_progress = st.slider("Прогресс %", 0.0, 100.0, float(proj.get("progress", 0)))
                    ...
                    if save:
                        result = api_client.update_project(str(proj["id"]), {
                            "title": ep_title, "description": ep_desc or None,
                            "lead": ep_lead or None, "status": ep_status,
                            "progress": ep_progress, "goal_id": ep_goal or None,
                        })
                        if "error" in result:
                            st.error(f"Ошибка: {result['error']}")
                        else:
...
TASK_STATUS_LABELS = {
    "todo": "К выполнению", "in_progress": "В работе", "in_review": "На ревью",
    "done": "Выполнено", "blocked": "Заблокировано",
}
...
with tab_tasks:
    ...
    employee_options = {str(e["id"]): e["full_name"] for e in employees} if employees else {}
    ...
    with st.expander("➕ Создать задачу", expanded=False):
        with st.form("create_task", clear_on_submit=True):
            ...
            with t_c1:
                t_status = st.selectbox("Статус", TASK_STATUSES, format_func=lambda x: TASK_STATUS_LABELS.get(x, x), key="t_status")
            with t_c2:
                t_priority = st.selectbox("Приоритет", TASK_PRIORITIES, index=2, key="t_priority")
            t_c3, t_c4 = st.columns(2)
            with t_c3:
                t_project = st.selectbox("Проект", [""] + project_ids, format_func=lambda x: project_options.get(x, "— Не выбран —"), key="t_project")
            with t_c4:
                t_assignee = st.selectbox("Исполнитель", [""] + employee_ids, format_func=lambda x: employee_options.get(x, "— Не выбран —"), key="t_assignee")
            ...
            if st.form_submit_button("Создать задачу", type="primary"):
                if not t_title:
                    ...
                    result = api_client.create_task({
                        "title": t_title, "description": t_desc or None,
                        "status": t_status, "priority": t_priority,
                        "project_id": t_project or None,
                        "assignee_id": t_assignee or None,
                        "story_points": t_sp, "external_id": t_ext or None,
                    })
                    if "error" in result:
                        st.error(f"Ошибка: {result['error']}")
                    else:
    ...
    if tasks:
        for task in tasks:
            t_emoji = {"done": "✅", "blocked": "🚫", "in_progress": "🔄", "in_review": "👀", "todo": "📋"}.get(task["status"], "📋")
            with st.expander(f"{t_emoji} {task['title']}"):
                with st.form(f"edit_task_{task['id']}"):
                    et_title = st.text_input("Название", value=task["title"])
                    et_desc = st.text_area("Описание", value=task.get("description") or "")
                    et_c1, et_c2 = st.columns(2)
                    with et_c1:
                        et_status = st.selectbox(
                            "Статус", TASK_STATUSES,
                    ...
                    with et_c2:
                        et_priority = st.selectbox(
                            "Приоритет", TASK_PRIORITIES,
                            index=TASK_PRIORITIES.index(task["priority"]) if task["priority"] in TASK_PRIORITIES else 2,
                        )
                    ...
                    with et_c3:
                        ...
                        et_project = st.selectbox(
                            "Проект", [""] + project_ids,
                            index=(project_ids.index(cur_proj) + 1) if cur_proj in project_ids else 0,
                            format_func=lambda x: project_options.get(x, "— Не выбран —"),
                        )
                    with et_c4:
                        cur_assign = str(task.get("assignee_id") or "")
                        et_assignee = st.selectbox(
                            "Исполнитель", [""] + employee_ids,
                            index=(employee_ids.index(cur_assign) + 1) if cur_assign in employee_ids else 0,
                            format_func=lambda x: employee_options.get(x, "— Не выбран —"),
                        )
                    et_sp = st.number_input("Story Points", min_value=0, value=task.get("story_points", 0))
                    ...
                    if save:
                        result = api_client.update_task(str(task["id"]), {
                            "title": et_title, "description": et_desc or None,
                            "status": et_status, "priority": et_priority,
...
with tab_employees:
            ...
            if st.form_submit_button("Добавить сотрудника", type="primary"):
                if not e_name or not e_email:
                    ...
                    result = api_client.create_employee({
                        "full_name": e_name, "email": e_email,
                        "department": e_dept or None, "position": e_pos or None,
                        "is_active": e_active,
                    })
                    if "error" in result:
                        st.error(f"Ошибка: {result['error']}")
    ...
    if employees:
        for emp in employees:
            ...
            with st.expander(f"{active_icon} {emp['full_name']} — {emp.get('position', '')}"):
                with st.form(f"edit_emp_{emp['id']}"):
                    ...
                    with ee_c1:
                        ee_dept = st.text_input("Отдел", value=emp.get("department") or "")
                    with ee_c2:
                        ee_pos = st.text_input("Должность", value=emp.get("position") or "")
                    ...
                    if save:
                        result = api_client.update_employee(str(emp["id"]), {
                            "full_name": ee_name, "email": ee_email,
                            "department": ee_dept or None, "position": ee_pos or None,
                            "is_active": ee_active,
                        })
                        if "error" in result:
                            st.error(f"Ошибка: {result['error']}")
...
DOC_TYPE_LABELS = {
    "specification": "Спецификация", "contract": "Контракт", "report": "Отчёт",
    "confluence_page": "Confluence", "email": "Письмо", "other": "Другое",
}
...
with tab_documents:
    ...
    with st.expander("📤 Загрузить файл", expanded=False):
        uploaded_file = st.file_uploader(
            ...
            type=["txt", "csv", "md", "json", "xml", "html", "log", "yml", "yaml", "py", "js", "sql"],
            key="doc_upload",
        )
        with st.form("upload_doc", clear_on_submit=True):
            ud_title = st.text_input("Название (по умолчанию — имя файла)", key="ud_title")
            ud_c1, ud_c2 = st.columns(2)
            with ud_c1:
                ud_type = st.selectbox("Тип документа", DOC_TYPES, index=5, format_func=lambda x: DOC_TYPE_LABELS.get(x, x), key="ud_type")
            with ud_c2:
                ud_author = st.text_input("Автор", key="ud_author")
            ud_project = st.selectbox(
                "Проект", [""] + project_ids,
                format_func=lambda x: project_options.get(x, "— Не выбран —"), key="ud_project",
            )
            ...
            if st.form_submit_button("📤 Загрузить", type="primary"):
                if not uploaded_file:
                    ...
                    result = api_client.upload_document(file_bytes, uploaded_file.name, **kwargs)
                    if "error" in result:
                        st.error(f"Ошибка: {result['error']}")
                    else:
    ...
    with st.expander("➕ Создать документ вручную", expanded=False):
        with st.form("create_doc_manual", clear_on_submit=True):
            ...
            with dm_c1:
                dm_type = st.selectbox("Тип", DOC_TYPES, index=5, format_func=lambda x: DOC_TYPE_LABELS.get(x, x), key="dm_type")
            with dm_c2:
                dm_status = st.selectbox("Статус", DOC_STATUSES, format_func=lambda x: DOC_STATUS_LABELS.get(x, x), key="dm_status")
            with dm_c3:
                dm_author = st.text_input("Автор", key="dm_author")
            dm_project = st.selectbox(
                "Проект", [""] + project_ids,
                format_func=lambda x: project_options.get(x, "— Не выбран —"), key="dm_project",
            )
            ...
            if st.form_submit_button("Создать документ", type="primary"):
                if not dm_title:
                    ...
                    result = api_client.create_document({
                        "title": dm_title, "content": dm_content or None,
                        "doc_type": dm_type, "status": dm_status,
    ...
    if documents:
        for doc in documents:
            doc_emoji = {"approved": "✅", "rejected": "❌", "draft": "📝", "review": "👀", "archived": "📦"}.get(doc["status"], "📄")
            type_label = DOC_TYPE_LABELS.get(doc.get("doc_type", ""), doc.get("doc_type", ""))
            with st.expander(f"{doc_emoji} {doc['title']} — {type_label}"):
                with st.form(f"edit_doc_{doc['id']}"):
                    ...
                    ed_content = st.text_area("Содержимое", value=doc.get("content") or "", height=150)
                    ed_c1, ed_c2, ed_c3 = st.columns(3)
                    with ed_c1:
                        ed_type = st.selectbox(
                            "Тип", DOC_TYPES,
                            index=DOC_TYPES.index(doc["doc_type"]) if doc.get("doc_type") in DOC_TYPES else 5,
                            format_func=lambda x: DOC_TYPE_LABELS.get(x, x),
                        )
                    with ed_c2:
                        ed_status = st.selectbox(
                            "Статус", DOC_STATUSES,
                            index=DOC_STATUSES.index(doc["status"]) if doc.get("status") in DOC_STATUSES else 0,
                            format_func=lambda x: DOC_STATUS_LABELS.get(x, x),
                        )
                    with ed_c3:
                        ed_author = st.text_input("Автор", value=doc.get("author") or "")
                    cur_doc_proj = str(doc.get("project_id") or "")
                    ed_project = st.selectbox(
                        "Проект", [""] + project_ids,
                        index=(project_ids.index(cur_doc_proj) + 1) if cur_doc_proj in project_ids else 0,
                        format_func=lambda x: project_options.get(x, "— Не выбран —"),
                    )
                    ...
                    if save:
                        result = api_client.update_document(str(doc["id"]), {
                            "title": ed_title, "content": ed_content or None,
                            "doc_type": ed_type, "status": ed_status,
